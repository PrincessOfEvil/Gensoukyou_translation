...
//        }
        ...
        float effect = (BASE_ANTI_ARMOR_EFFECT * (flux_level));
        ...
        ship.getMutableStats().getHitStrengthBonus().modifyPercent(ship.getFleetMemberId(),effect);
    }

    @Override
    public boolean isApplicableToShip(ShipAPI ship) {
        if (ship.getVariant().getHullMods().contains(FantasyBasicMod.FANTASYBASICMOD)) return true;
        return false;
    }
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, ShipAPI.HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        tooltip.addPara(I18nUtil.getHullModString("FantasyPlayerMod_DAE_0"), Misc.getGrayColor(), 4f);
    }

    public String getDescriptionParam(int index, ShipAPI.HullSize hullSize) {


        if (index == 0) return "" + (int)DAMAGE_TAKEN + "%";
        if (index == 1) return "" + (int)BASE_ANTI_ARMOR_EFFECT + "%";
