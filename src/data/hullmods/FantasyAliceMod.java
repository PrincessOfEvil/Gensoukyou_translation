    ...
    public void advanceInCombat(ShipAPI ship, float amount) {
        ...
        for (ShipAPI fighter : fighters){
            ...
            if (MathUtils.isWithinRange(fighter,ship,EFFECT_RANGE)){
                fighter.getMutableStats().getTimeMult().modifyMult(fighter.getFleetMemberId(),FIGHTER_TIME_MULT);

                fighter.setJitterUnder(fighter,EFFECT_COLOR,1f,6,10f);

            }else {
                fighter.getMutableStats().getTimeMult().unmodify(fighter.getFleetMemberId());
            }
