...
//    private boolean FULL_CHARGE = false;
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        out : for (ShipAPI ship_in_range : AIUtils.getAlliesOnMap(the_ship)) {

            float range = MathUtils.getDistance(the_ship, ship_in_range);
            if (range <= getSystemRange(the_ship)) {
        ...
        if (visual == null){
            ...
            visual.radius = getSystemRange(the_ship);
            visual.thickness = 30f;
            visual.color = VISYAL_COLOR;

            entityForVisual = Global.getCombatEngine().addLayeredRenderingPlugin(new FM_MisfortuneAbsorbVisual(visual));
        }
        if (visual != null && entityForVisual != null){
            entityForVisual.getLocation().set(the_ship.getLocation());
        }
        ...
        if (timerForParticles <= 0){
            for (int i = 0; i <15; i = i + 1){
                Global.getCombatEngine().addHitParticle(
                        MathUtils.getRandomPointInCircle(the_ship.getLocation(),getSystemRange(the_ship)),
