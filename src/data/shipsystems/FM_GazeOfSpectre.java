...
public class FM_GazeOfSpectre extends BaseShipSystemScript {
    ...
    public static final float INTENSITY = 36f;
    ...
    public void apply(MutableShipStatsAPI stats, final String id, State state, final float effectLevel) {
        ...
        if (effectLevel > 0 && !visualEffect){
            if (timer <= 2f){
                timer = timer + Global.getCombatEngine().getElapsedInLastFrame() * 1f;
            }
            if (ripple == null){
        ...
        if (effectLevel > 0){
            targetData.target.setJitterUnder(TARGET_INFO,JITTER_TARGET,effectLevel,20,10f);
